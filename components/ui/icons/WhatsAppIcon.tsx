...
function playFloatBurst(floatY: SharedValue<number>) {
  cancelAnimation(floatY);
  floatY.value = withSequence(
    withTiming(FLOAT_UP_PX, { duration: FLOAT_DURATION_MS, easing: Easing.inOut(Easing.sin) }),
    withTiming(0, { duration: FLOAT_DURATION_MS, easing: Easing.inOut(Easing.sin) }),
  );
}
...
export function WhatsAppIcon({ size = 18, color, disabled = false, hovered = false }: IconProps) {
  ...
  useEffect(() => {
    const id = setInterval(triggerFloat, FLOAT_INTERVAL_MS);
    return () => clearInterval(id);
  }, [triggerFloat]);

  useEffect(() => {
    if (hovered) triggerFloat();
  }, [hovered, triggerFloat]);
  ...
  return (
    <Animated.View style={[styles.wrap, { width: size, height: size }, animatedStyle]}>
      <WhatsAppGlyph size={size} color={iconColor} />
    </Animated.View>
  );
}
